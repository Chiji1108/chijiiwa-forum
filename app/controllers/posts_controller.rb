class PostsController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @post.save
        format.html { redirect_to my_thread_path(@my_thread), notice: 'ポストは正常に投稿されました！' }
  ...
  def update
    respond_to do |format|
      if @post.update(post_params)
        format.html { redirect_to my_thread_path(@my_thread), notice: 'ポストは正常に編集されました！' }
  ...
  def destroy
    @post.destroy
    respond_to do |format|
      format.html { redirect_to my_thread_path(@my_thread), notice: 'ポストは正常に削除されました！' }
